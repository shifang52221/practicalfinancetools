    ...
    <div className="calc-grid">
      <div className="panel">
        ...
        <div className="form">
          <div className="field field-6">
            <label className="label" htmlFor="debt-avalanche-extra-monthly">Extra payment (monthly)</label>
            <input id="debt-avalanche-extra-monthly" type="number" inputMode="decimal" value={extraMonthly} min={0} onChange={(e) => setExtraMonthly(+e.target.value)} />
          ...
          <div className="field field-6">
            <label className="label" htmlFor="debt-avalanche-target-months">Target debt-free time (months)</label>
            <input id="debt-avalanche-target-months" type="number" inputMode="numeric" value={targetMonths} min={1} step={1} onChange={(e) => setTargetMonths(+e.target.value)} />
                ...
                <div key={d.id} className="card2" style={{ padding: 12 }}>
                  <div className="form" style={{ gridTemplateColumns: "repeat(6, 1fr)" }}>
                    <div className="field field-2">
                      <label className="label" htmlFor={`debt-avalanche-name-${d.id}`}>Name</label>
                      <input
                        id={`debt-avalanche-name-${d.id}`}
                        value={d.name}
                        onChange={(e) => setDebts((prev) => prev.map((x) => (x.id === d.id ? { ...x, name: e.target.value } : x)))}
                      />
                    </div>
                    <div className="field field-2">
                      <label className="label" htmlFor={`debt-avalanche-balance-${d.id}`}>Balance</label>
                      <input
                        id={`debt-avalanche-balance-${d.id}`}
                        type="number"
                        inputMode="decimal"
                        value={d.balance}
                        min={0}
                    ...
                    <div className="field field-1">
                      <label className="label" htmlFor={`debt-avalanche-apr-${d.id}`}>APR (%)</label>
                      <input
                        id={`debt-avalanche-apr-${d.id}`}
                        type="number"
                        inputMode="decimal"
                        value={d.aprPercent}
                        min={0}
                    ...
                    <div className="field field-1">
                      <label className="label" htmlFor={`debt-avalanche-min-${d.id}`}>Min/mo</label>
                      <input
                        id={`debt-avalanche-min-${d.id}`}
                        type="number"
                        inputMode="decimal"
                        value={d.minPayment}
                        min={0}
      ...
      <div className="panel">
        ...
        <div className="btn-row" style={{ marginTop: 12 }}>
          ...
          <button className="btn" type="button" onClick={() => downloadCsv("debt-avalanche-payoffs.csv", toCsvPayoffs(plan.payoffs))}>
